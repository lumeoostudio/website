...
export const AnimatedChartUpIcon = (
	props: React.SVGProps<SVGSVGElement> & { delay?: number },
) => {
	...
	useGSAP(
		() => {
			...
			const fillPaths = gsap.utils
				.toArray<SVGPathElement>(
					svgRef.current.querySelectorAll("[data-fill-order]"),
				)
				.sort(
					(firstPath, secondPath) =>
						Number(firstPath.dataset.fillOrder) -
						Number(secondPath.dataset.fillOrder),
				);

			gsap.set(fillPaths, { fill: "transparent" });
