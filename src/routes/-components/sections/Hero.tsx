    ...
    <section className="mx-auto flex w-full flex-col items-center gap-25 py-20">
      <div className="flex w-full max-w-4xl flex-1 flex-col items-center gap-12">
        <div className="flex w-full flex-col items-center gap-4.5">
          <p className="w-full text-center font-medium text-[48px] leading-[1.1] tracking-[-2%]">
        ...
        <div className="flex items-center gap-6">
          <Button className="!max-h-14 box-content rounded-full px-6.25 py-3 text-xl leading-7.5">Book a Call</Button>
          <Button className="!max-h-14 box-content rounded-full px-6.25 py-3 text-xl leading-7.5" variant="tertiary">
            See Our Projects
          </Button>
        </div>
      </div>
      <div className="relative h-112 w-full overflow-hidden" onPointerEnter={onPointerEnter} onPointerLeave={onPointerLeave}>
        <ul ref={trackRef} className="absolute flex h-full w-max list-none items-center gap-8" aria-label="Hero showcase carousel">
          {HERO_MARQUEE_LOOP.map((image, index) => (
            <li key={`${image.src}-${index}`} className="h-full shrink-0">
              <img src={image.src} alt={image.alt} className="h-full w-auto object-cover" draggable={false} />
            </li>
          ))}
        </ul>
      </div>
