import { ParticleWaveField, type RippleOrigin } from "#/components/particle-wave-field";
...
const steps: WorkStep[] = [
  {
    step: "STEP 1",
    title: "Discover",
    description: "We start where others don't, real user insights that shape everything.",
    particle: {
      waveStrength: 1,
      rippleStrength: 1,
  ...
  },
  {
    step: "STEP 2",
    title: "Design",
    description: "Insight becomes form bold, intentional design informed by discovery.",
    particle: {
      waveStrength: 1.15,
      rippleStrength: 0.85,
  ...
  },
  {
    step: "STEP 3",
    title: "Deliver",
    description: "We build and ship scalable systems, no-code sites, and hand them off to you.",
    particle: {
      waveStrength: 0.92,
      rippleStrength: 1.15,
  ...
  return (
    <section id="process" className="mx-auto flex w-full max-w-340 flex-col items-stretch gap-16 px-4 py-10 sm:px-10 sm:py-30 lg:px-16">
      ...
      <ul className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {steps.map((step) => (
          <li key={step.step} className="relative h-123.75 overflow-hidden bg-[#F9F9FA]">
            <ParticleWaveField
            ...
            <div className="relative z-10 flex flex-col gap-3 p-6">
              <div className="flex flex-col gap-2">
                <p className="font-medium font-secondary text-secondary text-sm leading-[1.1]">{step.step}</p>
                <p className="font-medium text-4xl text-primary">{step.title}</p>
              </div>
