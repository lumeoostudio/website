...
export const Pricing = () => {
  ...
  const prod = productPlans.map((plan) => (
    <Card key={plan.title} className="flex-1 justify-between rounded-none bg-[#F9F9FA] p-6 ring-0 lg:p-8">
      <CardHeader className="gap-1 p-0">
        <CardTitle className="flex flex-col font-medium text-primary text-xl leading-normal tracking-[-0.5px]">{plan.title}</CardTitle>
        <p className="font-medium font-tertiary text-base text-primary/70 leading-normal tracking-[-0.5px]">{plan.description}</p>
      </CardHeader>
      <CardContent className="flex flex-1 flex-col gap-8 p-0 pb-8!">
        <div className="flex flex-1 flex-col gap-8">
          <div className="flex flex-col gap-1">
            <p className="font-medium font-tertiary text-base leading-normal tracking-[-0.5px]">{plan.startingFromLabel}</p>
            <p className="font-medium text-[40px] text-base leading-normal tracking-[-0.5px]">{plan.price}</p>
          </div>
          <ul className="flex flex-col gap-3">
            {plan.features.map((feature) => (
              <li key={feature} className="flex items-start gap-3">
                <div className="mt-1 size-4.5 shrink-0 bg-primary" />
                <p className="text-base text-primary tracking-[-0.5px]">{feature}</p>
              </li>
            ))}
          </ul>
        </div>
  ...
  return (
    <section id="pricing" className="mx-auto flex w-full max-w-340 flex-col items-stretch gap-12 px-4 py-10 sm:px-10 sm:py-30 lg:px-16">
      <div className="flex flex-col gap-16">
        ...
        <Tabs defaultValue="product" className="flex flex-col gap-8">
          ...
          <TabsContent value="product" className="flex flex-col gap-6 lg:flex-row">
            {prod}
          </TabsContent>
          <TabsContent value="landing" className="flex flex-col gap-6 lg:flex-row">
            {prod}
          </TabsContent>
          <TabsContent value="branding" className="flex flex-col gap-6 lg:flex-row">
            {prod}
          </TabsContent>
        </Tabs>
      </div>
