    ...
    <section className="mx-auto flex w-full max-w-340 flex-col items-stretch gap-16 px-16 py-30">
      ...
      <ul className="grid grid-cols-3 gap-6">
        {testimonials.map((testimonial, index) => (
          <li key={`${testimonial.name}-${index}`} className="flex max-h-h-71.25 min-h-71.25 flex-col justify-between gap-4 bg-[#F9F9FA] p-6">
            <p className="font-tertiary text-primary/70 leading-[1.4] tracking-[-3%]">{testimonial.quote}</p>
            <div className="flex items-center gap-3">
              <img src="/assets/testimonials/testimonial-1.png" alt="Testimonial 1" />
