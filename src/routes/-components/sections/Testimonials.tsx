...
function TestimonialVideo({
  ...
  useEffect(() => {
    ...
    el.pause();
  }, [isInView, onRequestPlay, videoId]);
  ...
  return (
    <div ref={containerRef} className="group relative max-h-82.5 min-h-82.5 w-full">
      <video
        ref={videoRef}
        src={src}
        loop
      ...
      >
        <track kind="captions" srcLang="en" label="English captions" src="data:text/vtt;charset=utf-8,WEBVTT" />
      </video>
      <button
        type="button"
      ...
      >
        <HugeiconsIcon icon={isPlaying ? PauseIcon : PlayIcon} className="size-5 fill-white text-white" />
      </button>
    </div>
  );
}
...
const testimonials = [
  {
    ...
    video: "https://res.cloudinary.com/dky0e8lq6/video/upload/testimonial-1_tj4ksc.mp4",
      ...
      <div className="overflow-hidden" onPointerEnter={onPointerEnter} onPointerLeave={onPointerLeave}>
        <ul ref={trackRef} className="m-0 flex w-max list-none gap-6 p-0" aria-label="Partner testimonials">
          {MARQUEE_LOOP.map((testimonial, index) => (
            <li key={`${testimonial.name}-${index}`} className="flex min-h-71.25 max-w-100 shrink-0 flex-col justify-between gap-4 bg-[#F9F9FA] p-6 sm:min-w-95">
              {testimonial.video ? (
                <TestimonialVideo src={testimonial.video} videoId={`${testimonial.name}-${index}`} activeVideoId={activeVideoId} onRequestPlay={setActiveVideoId} />
              ) : (
                <p className="font-tertiary text-primary/70 leading-[1.4] tracking-[-3%]">{testimonial.quote}</p>
              )}

              <div className="flex items-center gap-3">
                <img src={testimonial.image} alt="" width={48} height={48} className="size-12 shrink-0 rounded-none object-cover" />
